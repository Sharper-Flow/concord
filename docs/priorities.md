...
1. The operator can see, plan, and act across the full Product scope from the launcher through Product → Domain context. Per CD-0021, "across the full Product scope" means every Product is reachable from the launcher, not that a result set spans Products; and "plan" is satisfied by reaching work and opening a session that authors it, since the launcher is read-only by C18 and work creation keeps one write authority.
