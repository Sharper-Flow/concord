...
| [`terminal-launcher-contract.md`](./terminal-launcher-contract.md) | **Accepted C18 contract; S1 portfolio slice implemented, shipped only after merge:** read-only Product launcher — screen set, navigation graph, knowledge-in-context, ambient Product context, identity-only launch handoff, no-poll refresh model, Bubble Tea v2 adapter, and Product-only query scope. S2/S3/session launch remain unbuilt. |
...
| [`product-coordination-view.md`](./product-coordination-view.md) | **Open C17 candidate; non-authorizing design input.** Proposed Product coordination drill-down behind the C14 row: a structural Q8 relation tree plus a ranked Q5/Q4 work table. Binds nothing until accepted. |
