...
func TestResolveCompactionHomeUsesProductThenPrimaryMembership(t *testing.T) {
	...
	if _, err := s.DatabaseForTesting().Exec(`INSERT INTO product_knowledge_homes(product_id,project_id,locator_id) VALUES('prod','proj','locator-1')`); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := s.DatabaseForTesting().Exec(`DELETE FROM product_knowledge_homes WHERE product_id='prod'`); err != nil {
		t.Fatal(err)
	}
