...
test("spawn failure is a typed blocked outcome with bounded diagnostic", async () => {
  let argv: string[] = []
  const result = await dispatchWorker(packet(), { credentials: testCredentials, binary: "opencode-test", runner: { async run(args) { argv = args; throw new Error("spawn failed") } } })
...
test("the adapter does not declare a model — argv carries no --model", async () => {
  let argv: string[] = []
  await dispatchWorker(packet(), { credentials: testCredentials, runner: { async run(args) { argv = args; return { exitCode: 0, stdout: runOutput(), stderr: "" } } }, evidenceRunner: { async run() { return { exitCode: 0, stdout: "", stderr: "" } } } })
...
test("the readback shape is recorded verbatim regardless of host configuration", async () => {
  ...
  const result = await dispatchWorker(packet(), { credentials: testCredentials, runner: workerRunner(hostExecuted) })
  expect(result.outcome).toBe("ok")
