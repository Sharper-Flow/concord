...
def validate(manifest: dict) -> str:
    ...
    global_budget_seconds = manifest.get("bounds", {}).get("max_budget_seconds")
    if global_budget_seconds != 300:
        fail("global operation budget ceiling is not canonical")
    ...
    for op in operations:
        if set(op) - {"id", "tool", "kind", "availability", "query_id", "input_schema", "result_schema", "capability", "consequence", "approval", "supported_budget_seconds", "metadata"}:
            fail(f"operation section is not closed: {op.get('id')}")
        ceiling = op.get("supported_budget_seconds")
        if not isinstance(ceiling, int) or isinstance(ceiling, bool) or not 1 <= ceiling <= global_budget_seconds:
            fail(f"operation must declare one supported budget within the global ceiling: {op.get('id')}")
...
def go_projection(manifest: dict, digest: str) -> str:
    ...
    consequence_values = ", ".join(json.dumps(value) for value in manifest["consequences"])
    global_budget_seconds = manifest["bounds"]["max_budget_seconds"]
    payload = json.loads(PAYLOAD.read_text())
    rules = []
...
def main() -> int:
    try:
        ...
        expected = {
            ...
            ROOT / "adapter/opencode/generated-contracts.ts": ts_projection(manifest, digest),
