...
func TestAlignmentStepHasOnlyRecordAlignmentAsItsAdvanceExit(t *testing.T) {
	...
	cases := []struct {
		...
		{"workflow.implementation", "proposal", "discovery", 15},
		{"workflow.break_fix", "reproduce", "diagnose", 13},
	}
...
func TestPriorPinnedDefinitionVersionsReplayUnchanged(t *testing.T) {
	...
	for pin, digest := range workflowDefinitionVersionPins {
		...
		if stepDeclaresAction(definition, "alignment", "record_alignment") && pinVersion(t, version) != current[ref]-1 && !(ref == "workflow.break_fix" && pinVersion(t, version) == 11) && !(ref == "workflow.implementation" && pinVersion(t, version) == 13) {
		...
		for _, step := range definition.StepGraph.Steps {
			if step.ID == "alignment" && !(ref == "workflow.break_fix" && pinVersion(t, version) == 11) && !(ref == "workflow.implementation" && pinVersion(t, version) == 13) {
