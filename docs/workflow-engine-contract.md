...
| **Architecture spike** — [`architecture-spike.md` §2](./architecture-spike.md#2-shape) | `frame[frame_question, approve_contract] → research[record_research, bind_evidence] → options[record_option] → poc_optional[start_poc, checkpoint_poc, discard_poc, record_delivery, accept_worker_result] → decision_record[record_decision] → review[record_verdict, accept_decision] → acceptance[confirm_premise] → complete[complete]`. The optional POC edge skips only `poc_optional`; completion still requires the accepted decision record, verdict, premise confirmation, and `complete`. |
| **Ops runbook** — [`workflows.md` §4](./workflows.md#4-example-workflow-types) and [`managed-resource-inventory.md` §3](./managed-resource-inventory.md#3-stage-rule) | `plan[approve_contract, resolve_condition, cancel_condition] → approval[approve_operation, resolve_condition, cancel_condition] → execute[start_run, checkpoint_run, bind_evidence, add_condition, resolve_condition, cancel_condition, record_delivery, accept_worker_result] → health[record_health, record_verdict, resolve_condition, cancel_condition] → rollback_optional[rollback_run, resolve_condition, cancel_condition, record_delivery, accept_worker_result] → cleanup[cleanup_run, confirm_premise, resolve_condition, cancel_condition] → complete[complete, resolve_condition, cancel_condition]`. Rollback is optional recovery, not a second completion path. |
| **Static analysis** — [`workflows.md` §4–§5](./workflows.md#5-coordinating-external-analysis-tools) | `scope[approve_contract, declare_scope] → analyze[run_analysis, checkpoint_analysis, record_delivery, accept_worker_result] → report[record_report, bind_evidence] → review[record_verdict, confirm_premise] → complete[complete]`. Completion verb: report exists over the declared surface. |
| **Generic one-off** — [`workflows.md` §2.1a](./workflows.md#21a-outcome-contract) and CD-0013 D14 | `define[approve_contract] → execute[start_action, checkpoint_action, bind_evidence, link_successor, record_delivery, accept_worker_result] → verify[record_verdict, confirm_premise] → complete[complete]`. It may use any operator-authored allowed predicate without copying another family's semantics. |
...
| `record_delivery` | `workflow.action_completed`, which advances the external-effect step when its fenced start ran and no lane attempt was dispatched in that attempt; a dispatched attempt exits through `accept_worker_result` |
