...
a completed `durable_operations` row with `op_id` `worktree_verify:<lease_id>`,
the requesting principal and request identity, and the run reference in
`evidence_refs`. `bind_evidence` names that row as its producer. The
acceptance checkpoint's verification requirement then resolves against the
core run that produced it. A failed or mutated run records no authority. The
gate keeps refusing unbound verification.
...
| `bind_evidence` | `workflow.evidence_bound`; the producer names a completed durable operation, for example a green verify-tier run at `worktree_verify:<lease_id>` |
