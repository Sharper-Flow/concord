...
func seedNativeRunFixture(t *testing.T, s *Store) string {
	...
	if err := s.Transact(ctx, func(tx *Transaction) error {
		events := []Event{
			...
			{EventID: "gate-work", Kind: "work.created", SubjectType: SubjectWorkItem, SubjectID: "work-gate", Actor: "operator", OccurredAt: time.Now().UTC(), PayloadVersion: 2, Payload: []byte(`{"work_kind":"ops_runbook","title":"Gate","priority":1}`)},
