...
| `workflow.backlog_alignment_recorded` | `work_id`, `searched` (1–4096, the search performed and its scope), `outcome` (`related_found` or `none_found`), `related_ids` (1–64 unique work ids, present only when `outcome` is `related_found`; the fold refuses `related_found` with no ids and `none_found` with ids, and every id must name a real work item) |
