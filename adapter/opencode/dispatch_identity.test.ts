...
test("run metadata accepts only one typed session identity", () => {
  ...
  expect(readRunSessionMetadata(`${JSON.stringify({ ts: "2026-08-22T05:08:17.270Z", level: "info", plugin: "opencode-model-routing", event: "config.loaded" })}\n${output}`)).toEqual({ session_id: "session-1", fallback_reason: null })
  expect(readRunSessionMetadata(`${output}\n${JSON.stringify({ type: "unknown", sessionID: "session-2" })}`)).toEqual({ session_id: "session-1", fallback_reason: null })
  expect(readRunSessionMetadata(`${output}\nplain host chatter that is not JSON`)).toEqual({ session_id: "session-1", fallback_reason: null })
})
