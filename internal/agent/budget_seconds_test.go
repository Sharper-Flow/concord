...
func TestRequestedBudgetSecondsJoinsTheCanonicalDigest(t *testing.T) {
	...
	one := mutationDigest("concord_work_transition", "workflow_action", CallEnvelope{GrantRef: "grant", ClientRef: "client", SessionRef: "session", AgentRef: "agent"}, json.RawMessage(`{"work_id":"w","expected_version":1,"action_id":"a","idempotency_key":"k","requested_budget_seconds":30}`))
	two := mutationDigest("concord_work_transition", "workflow_action", CallEnvelope{GrantRef: "grant", ClientRef: "client", SessionRef: "session", AgentRef: "agent"}, json.RawMessage(`{"work_id":"w","expected_version":1,"action_id":"a","idempotency_key":"k","requested_budget_seconds":20}`))
	if one == two {
