...
# The installer owns how the shipped adapter set is derived. Import that
# derivation rather than restating a list: a restated copy drifts, which is
# how an adapter shipped an import to a module the installer never placed.
# The derivation runs on the repository adapter directory here, which is the
# same canonical import-closure walk the installer runs on the verified
# release bundle at install time.
...
if _installer_spec is None or _installer_spec.loader is None:
    raise RuntimeError("unable to load the installer adapter file derivation")
...
ADAPTER_FILES = _installer.derive_adapter_files(ROOT / "adapter" / "opencode")
...
def check_workflow_contracts() -> list[str]:
    ...
    try:
        ...
        if len(scenarios) != 55:
            findings.append(f"scenarios/workflow-engine.v1.json: expected 55 scenarios, got {len(scenarios)}")
        ...
        if numbers != list(range(1, 56)):
            findings.append("scenarios/workflow-engine.v1.json: scenario numbers must be ordered 1..55")
...
''')
            ...
            if adapter.returncode:
                print("shipped adapter file set is not import-closed; see scripts/install.py derive_adapter_files", file=sys.stderr)
