...
func applyWorkEvent(t *testing.T, s *Store, event Event, expected map[string]int64) error {
	t.Helper()
...
func seedWork(t *testing.T, s *Store, id string) {
	t.Helper()
	if err := applyWorkEvent(t, s, workCreatedEvent(id, "create-"+id), map[string]int64{id: 0}); err != nil {
...
func seedState(t *testing.T, s *Store, id, state string) int64 {
	...
	case "needed":
		return 1
	case "in_progress":
		if err := applyWorkEvent(t, s, workTransitionEvent("start-"+id, id, "needed", "in_progress", 1, 2), map[string]int64{id: 1}); err != nil {
			t.Fatalf("start work %s: %v", id, err)
		}
		return 2
	case "completed", "cancelled":
		if err := applyWorkEvent(t, s, workTransitionEvent("finish-"+id, id, "needed", state, 1, 2), map[string]int64{id: 1}); err != nil {
			t.Fatalf("finish work %s: %v", id, err)
		}
		return 2
	case "superseded":
		seedWork(t, s, "successor-"+id)
		if err := applyWorkEvent(t, s, workSupersededEvent("supersede-"+id, "successor-"+id, id, 1, 2), map[string]int64{id: 1}); err != nil {
...
func TestLifecycleTransitionsAreClosedAndTyped(t *testing.T) {
	...
	for _, from := range states {
		for _, to := range states {
			...
			t.Run(name, func(t *testing.T) {
				...
				err := applyWorkEvent(t, s, workTransitionEvent("transition", "work", from, to, version, version+1), map[string]int64{"work": version})
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := applyWorkEvent(t, s, workReopenedEvent("reopen", "work", tc.from, version, version+1), map[string]int64{"work": version}); err != nil {
...
func TestWorkLifecycleRejectsStaleExpectedVersionWithoutMutation(t *testing.T) {
	s := openTemp(t)
	seedWork(t, s, "work")
	err := applyWorkEvent(t, s, workTransitionEvent("stale", "work", "needed", "in_progress", 1, 2), map[string]int64{"work": 0})
	...
	if lifecycle != "needed" || version != 1 {
		t.Fatalf("stale lifecycle mutation = %q version %d, want needed version 1", lifecycle, version)
	}
}
...
func TestPM4EvidenceFieldsRequireReasons(t *testing.T) {
	...
	t.Run("relation removed", func(t *testing.T) {
		...
		seedWork(t, s, "a")
		seedWork(t, s, "b")
		if err := applyWorkEvent(t, s, relationAddedEvent("add", "blocks", "a", "b", 1, 2), nil); err != nil {
			t.Fatal(err)
		}
		before := fullPM4Snapshot(t, s)
...
func TestRelationAddedRejectsStaleExpectedVersionWithoutMutation(t *testing.T) {
	...
	assertFailureKind(t, err, KindVersionConflict)
	assertTableCount(t, s, "domain_events", 2)
	...
	if version != 1 {
		t.Fatalf("stale relation mutation version = %d, want 1", version)
	}
}
...
func TestRelationRemovedRejectsStaleExpectedVersionWithoutMutation(t *testing.T) {
	...
	before := fullPM4Snapshot(t, s)
	err := applyWorkEvent(t, s, relationRemovedEvent("stale", "blocks", "a", "b", 1, 2), nil)
	assertFailureKind(t, err, KindVersionConflict)
	if got := fullPM4Snapshot(t, s); got != before {
...
func TestSupersessionIsAtomicAndRejectsCyclesAndSecondSuccessors(t *testing.T) {
	...
	if err := applyWorkEvent(t, s, workSupersededEvent("a-b", "a", "b", 1, 2), map[string]int64{"b": 1}); err != nil {
	...
	err := applyWorkEvent(t, s, workSupersededEvent("b-a", "b", "a", 1, 2), map[string]int64{"a": 1})
	assertFailureKind(t, err, KindCycleDetected)
	assertTableCount(t, s, "domain_events", 4)
	assertTableCount(t, s, "relations", 1)

	err = applyWorkEvent(t, s, workSupersededEvent("c-b", "c", "b", 2, 3), map[string]int64{"b": 2})
...
func TestSupersessionRejectsAlreadySupersededAndReopenRequiresCompositeEvent(t *testing.T) {
	...
	err := applyWorkEvent(t, s, workSupersededEvent("again", "c", "b", 2, 3), map[string]int64{"b": 2})
	assertFailureKind(t, err, KindSupersessionTargetAlreadySuperseded)

	err = applyWorkEvent(t, s, workTransitionEvent("direct-reopen", "b", "superseded", "needed", 2, 3), map[string]int64{"b": 2})
	assertFailureKind(t, err, KindIllegalLifecycleTransition)

	if err := applyWorkEvent(t, s, workReopenedFromSupersededEvent("composite-reopen", "b", "successor-b", 2, 3), map[string]int64{"b": 2}); err != nil {
...
func TestRelationKindsEnforceTheirOwnGraphRules(t *testing.T) {
	for _, kind := range []string{"parent", "blocks"} {
		t.Run(kind+" cycle", func(t *testing.T) {
			...
			for i, edge := range [][2]string{{"a", "b"}, {"b", "c"}} {
				if err := applyWorkEvent(t, s, relationAddedEvent(fmt.Sprintf("edge-%d", i), kind, edge[0], edge[1], 1, 2), nil); err != nil {
					t.Fatalf("add edge %v: %v", edge, err)
				}
			}
			assertFailureKind(t, applyWorkEvent(t, s, relationAddedEvent("cycle", kind, "c", "a", 1, 2), nil), KindCycleDetected)
	...
	for _, edge := range [][2]string{{"a", "b"}, {"b", "a"}} {
		if err := applyWorkEvent(t, s, relationAddedEvent(edge[0]+edge[1], "implements", edge[0], edge[1], 1, 2), nil); err != nil {
...
func TestCycleCheckRejectsSimpleTwoNodeCycle(t *testing.T) {
	...
	seedWork(t, s, "a")
	seedWork(t, s, "b")
	if err := applyWorkEvent(t, s, relationAddedEvent("a-b", "blocks", "a", "b", 1, 2), nil); err != nil {
		t.Fatal(err)
	}
	assertFailureKind(t, applyWorkEvent(t, s, relationAddedEvent("b-a", "blocks", "b", "a", 1, 2), nil), KindCycleDetected)
}
...
func TestRelationsRejectSelfDuplicateAndSupersedesContract(t *testing.T) {
	...
	assertFailureKind(t, applyWorkEvent(t, s, relationAddedEvent("self", "parent", "a", "a", 1, 2), nil), KindRelationConflict)
	if err := applyWorkEvent(t, s, relationAddedEvent("ab", "parent", "a", "b", 1, 2), nil); err != nil {
		t.Fatal(err)
	}
	assertFailureKind(t, applyWorkEvent(t, s, relationAddedEvent("ab-again", "parent", "a", "b", 2, 3), nil), KindRelationConflict)
	assertFailureKind(t, applyWorkEvent(t, s, relationAddedEvent("bad-supersedes", "supersedes", "a", "b", 2, 3), nil), KindRelationContractViolation)
}
...
func TestRebuildRestoresWorkAndRelationProjectionsByteForByte(t *testing.T) {
	...
	operations := []Operation{
		{Events: []Event{relationAddedEvent("parent", "parent", "a", "b", 1, 2)}},
		{Events: []Event{relationRemovedEvent("remove-parent", "parent", "a", "b", 2, 3)}},
		{Events: []Event{relationAddedEvent("parent-again", "parent", "a", "b", 3, 4)}},
		{Events: []Event{relationAddedEvent("blocks", "blocks", "b", "c", 1, 2)}},
		{Events: []Event{relationAddedEvent("implements", "implements", "c", "a", 1, 2)}},
		{Events: []Event{workSupersededEvent("supersede", "a", "c", 2, 3)}, ExpectedVersions: map[string]int64{"c": 2}},
	}
...
func TestRelationRemovalAndFoldGuard(t *testing.T) {
	...
	seedWork(t, s, "a")
	seedWork(t, s, "b")
	if err := applyWorkEvent(t, s, relationAddedEvent("ab", "blocks", "a", "b", 1, 2), nil); err != nil {
		t.Fatal(err)
	}
	if err := applyWorkEvent(t, s, relationRemovedEvent("remove-ab", "blocks", "a", "b", 2, 3), nil); err != nil {
		t.Fatal(err)
	}
	assertTableCount(t, s, "relations", 0)
	assertFoldGuardEmpty(t, s)
	if err := applyWorkEvent(t, s, relationAddedEvent("ab-again", "blocks", "a", "b", 3, 4), nil); err != nil {
		t.Fatal(err)
	}
