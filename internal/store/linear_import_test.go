...
func TestLinearInitiativeImportRecordsTheProjectLink(t *testing.T) {
	...
	op, err := s.EnqueueLinearIssueForWork(ctx, "import-link-entry", LinearOpIssueUpdate)
	if err != nil {
		t.Fatalf("EnqueueLinearIssueForWork(entry) error = %v", err)
	}
	payload := decodeLinearPayload(t, s, op.OperationID)
	if payload.ProjectID != "ini-uuid-link" {
		t.Fatalf("entry payload project id = %q, want the imported ini-uuid-link", payload.ProjectID)
	}
}
