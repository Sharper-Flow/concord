...
// partialNativeRunEnvelope renders the TS7 partial outcome for a workflow
// action whose typed native report showed the approved logical operation did
// not complete successfully (CD-0039 D7/D8). Completed steps are the run's
// reported phases in recorded order — never inferred from prose — and ok is
// reserved for successful native predicates.
func partialNativeRunEnvelope(base Envelope, execution store.WorkflowActionExecutionResult, changed []ChangedRef) Envelope {
	...
	ref := OperationRef{ID: execution.OperationID, Kind: "workflow_action", Version: "1", State: state, CurrentStep: "execute", UpdatedAt: time.Now().UTC()}
	completed := []string{}
	var failedStep string
	result := map[string]any{}
	if raw := string(execution.Result); raw != "" {
		_ = json.Unmarshal(execution.Result, &result)
	}
	for _, run := range execution.NativeRuns {
		switch run.Phase {
		case store.NativePhaseStart:
			completed = append(completed, "start_run")
			if run.Status == "failed_to_start" {
				failedStep = "start_run"
			}
		case store.NativePhaseHealth:
			completed = append(completed, "record_health")
			if run.Status == "failed" || run.Status == "degraded" {
				failedStep = "record_health"
				result["health_failure"] = map[string]any{"run_id": run.RunID, "status": run.Status, "observation_id": run.ObservationID, "evidence_ref": run.EvidenceRef, "evidence_digest": run.EvidenceDigest, "asserted_at": run.AssertedAt, "reporting_authority_ref": run.ReportingAuthorityRef}
			}
		case store.NativePhaseRollback:
			completed = append(completed, "rollback_run")
			if run.Status == "rollback_failed" {
				failedStep = "rollback_run"
			}
			result["rollback_result"] = map[string]any{"run_id": run.RunID, "status": run.Status, "evidence_ref": run.EvidenceRef, "evidence_digest": run.EvidenceDigest, "asserted_at": run.AssertedAt, "reporting_authority_ref": run.ReportingAuthorityRef}
		case store.NativePhaseCleanup:
			completed = append(completed, "cleanup_run")
			if run.Status == "cleanup_failed" {
				failedStep = "cleanup_run"
			}
		}
	}
	if len(completed) == 0 {
		completed = []string{"start_run"}
	}
	payload, err := json.Marshal(result)
	if err != nil {
		payload = execution.Result
	}
	envelope := NewPartial(base, ref, completed, TypedError{Kind: "operation_conflict", RetrySafe: retrySafe, RecoveryAction: RecoveryAction{Kind: "reconcile_operation", RequiredRefs: []string{"operation_id"}}, EffectState: EffectPartial})
	envelope.FailedStep = failedStep
	envelope.Result = payload
	envelope.ChangedRefs = changed
	return envelope
}
...
func (r runtime) mutateWorkflowAction(ctx context.Context, base Envelope, raw []byte, grant Grant, op ContractOperation) (Envelope, error) {
	...
	err = store.AuthorizeWorkflowActionAtBoundaryTx(ctx, r.Store, registry, store.WorkflowActionPreflightRequest{WorkID: in.WorkID, ExpectedVersion: in.ExpectedVersion, ActionID: in.ActionID, SelectedChoice: in.SelectedChoice, DecisionContextDigest: in.DecisionContextDigest, Payload: payload, Actor: actionRequest.Actor}, nil, time.Time{}, func(tx *store.Transaction) error {
		...
		if execution.ResultKind == "partial" || execution.ResultKind == "failed" {
			// CD-0039 D7: the core derives partial from the typed report in
			// the same transaction; the adapter cannot synthesize it. The
			// caller receives the same operation ID, the native steps that
			// actually completed, and the failure and rollback results.
			result = partialNativeRunEnvelope(base, execution, changed)
		} else {
