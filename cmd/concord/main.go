...
var commandSpecs = []commandSpec{
	...
	{Canonical: "worker-complete", RequiredFields: requiredFields(field("event_id"), field("work_id"), field("attempt_id"), field("readback_model"), field("report_schema_version"), field("evidence_origin")), Optional: "review_result (review lane only); evidence[]", Enums: "evidence_origin: reported | legacy_unavailable; review_result: pass | block; evidence[].obligation: bounded_findings | commands | contract_findings | exit_codes | failure_classification | files_touched | severity | source_citations | uncertainties | unresolved_issues | verification_commands | visual_artifacts"},
	...
	{Canonical: "session-prepare", RequiredFields: requiredFields(field("product_id"), field("work_id")), Optional: "task (bounded to 8192 bytes; a resume supplies none)", Enums: "none"},
...
func runWorkerCommand(command string, raw []byte, s *store.Store, service *agent.Service, clock func() time.Time, out, errOut io.Writer) int {
	...
	case "worker-complete":
		...
		payload := store.WorkerCompletedPayload{AttemptID: request.AttemptID, ReadbackModel: request.ReadbackModel, ReportSchemaVersion: request.ReportSchemaVersion, Evidence: request.Evidence, EvidenceOrigin: request.EvidenceOrigin, ReviewResult: request.ReviewResult}
