...
// block first, bounded by limit (<=100). Product scoping follows the
// challenge's captured Product scope: when products is non-empty, only
// challenges whose product_scope_json intersects it are listed. An empty
// products list lists
...
func (s *Store) BlockedSessions(ctx context.Context, now time.Time, products []string, limit int) (BlockedSessionsResult, error) {
	...
	// The projection reads one bounded challenge result inside a transaction.
...
func blockedSessionsCore(ctx context.Context, q queryer, now time.Time, products []string, limit int) (BlockedSessionsResult, error) {
	...
	rows, err := q.QueryContext(ctx, `
		SELECT session_ref, agent_ref, worktree, directory, consequence, issued_at, product_scope_json
		FROM agent_approval_challenges
	...
	for rows.Next() {
		...
		if err := rows.Scan(&session.SessionRef, &session.AgentRef, &session.Worktree, &session.Directory, &session.Consequence, &session.BlockedSince, &productJSON); err != nil {
		...
		if len(want) > 0 {
			var sessionProducts []string
			if json.Unmarshal([]byte(productJSON), &sessionProducts) != nil {
				continue
			}
			matched := false
			for _, product := range sessionProducts {
				if want[product] {
					matched = true
					break
				}
