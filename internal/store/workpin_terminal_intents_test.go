...
// the item, and WorkflowActionPreflight then refuses every workflow action
...
func TestTerminalWorkOffersNoWorkflowIntent(t *testing.T) {
	...
	refusal := WorkflowActionPreflight(ctx, s, WorkflowActionPreflightRequest{WorkID: "workpin-terminal", ActionID: live.NextValidIntents[0].ActionID})
	if refusal == nil {
