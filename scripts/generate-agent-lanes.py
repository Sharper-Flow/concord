...
def report_projection_constraints(report_schema: dict) -> list[str]:
    properties = report_schema["properties"]
    evidence_entry = report_schema["$defs"]["evidence_entry"]
    evidence_obligation = report_schema["$defs"]["evidence_obligation"]
    return [
        ...
        f"enum={json.dumps(evidence_obligation['enum'], ensure_ascii=False)}.",
    ]
...
def ts_projection(manifest: dict, manifest_digest: str, packet_schema: dict, report_schema: dict) -> str:
    report_constraints = report_projection_constraints(report_schema)
...
def agent_projection(lane: dict, report_schema: dict | None = None) -> str:
    ...
    report_constraints = "\n".join(f"- {item}" for item in report_projection_constraints(report_schema))
    return f"""---
