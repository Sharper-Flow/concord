...
| `workflow.staleness_observed` | `rule_id`, `severity` (`warning` or `block`), `drifted` (boolean), `observed_at` (RFC3339); the latest event for each rule is the staleness authority used by completion and reads |
