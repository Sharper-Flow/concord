...
def load_routing_policy(manifest: dict) -> tuple[dict, str]:
    ...
    for lane in manifest["lanes"]:
        ...
        if capability_class not in lane_classes:
            lane_classes[capability_class] = []
            lane_class_order.append(capability_class)
        lane_classes[capability_class].append(lane["pinned_model"])
    ...
    for entry in policies:
        ...
        if any(pinned_model != entry["preferred_model"] for pinned_model in lane_classes[capability_class]):
            raise ValueError(f"routing policy {capability_class} preferred model does not match every lane pinned_model")
...
def ts_projection(manifest: dict, manifest_digest: str, packet_schema: dict, policy: dict, policy_digest: str) -> str:
...
""" % (json.dumps(manifest_digest), json.dumps(manifest["lanes"], ensure_ascii=False, indent=2), json.dumps(packet_schema, ensure_ascii=False, separators=(",", ":")), json.dumps(policy_digest), json.dumps(policy["version"]), json.dumps(policy["policies"], ensure_ascii=False, indent=2))
...
def docs_projection(manifest: dict, manifest_digest: str) -> str:
    lines = ["# Concord agent lane registry", "", "<!-- Code generated by scripts/generate-agent-lanes.py; DO NOT EDIT. -->", "", f"Registry digest: `{manifest_digest}`", "", "| Lane | Capability class | Packet | Report | Pinned preferred model |", "|---|---|---|---|---|"]
    for lane in manifest["lanes"]:
    ...
    lines.extend(["", "Every lane is closed, versioned, digest-pinned, and pinned to a preferred model. Declared fallback resolution is governed by the generated routing policy. Unknown lane identity or digest fails closed before work begins.", ""])
    return "\n".join(lines)
...
def routing_docs_projection(policy: dict, policy_digest: str) -> str:
    ...
    for entry in policy["policies"]:
        resolution = " → ".join(f"`{model}`" for model in entry["resolution_set"])
        lines.append(f"| `{entry['capability_class']}` | `{entry['preferred_model']}` | {resolution} |")
    lines.extend([
        "",
...
def main() -> int:
    ...
    try:
        ...
        expected = {
            ...
            ROOT / "adapter/opencode/generated-agent-lanes.ts": ts_projection(manifest, manifest_digest, packet_schema, policy, policy_digest),
