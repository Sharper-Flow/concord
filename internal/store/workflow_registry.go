...
// authoredWorkflowDefinitionVersion is the version every builtin family ships
// in. A definition whose content changes must carry a new version here while
// the released prior version stays registered verbatim in workflow_history.go,
// because every live workflow instance pins the digest of the version it was
// captured under (issue #861).
...
// authoredWorkflowDefinitionVersion. Released prior versions are retained
// verbatim in workflow_history.go so instances pinned to them keep verifying;
// changing a family's content requires a new version here, never an in-place
...
func NewBuiltinWorkflowRegistry() DefinitionRegistry {
	...
	// Historical versions register first so the monotonic-version guard sees
	// the authored version as the highest registered for each family.
