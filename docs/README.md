...
| [`terminal-launcher-contract.md`](./terminal-launcher-contract.md) | **Accepted C18 contract; implementation unbuilt:** read-only Product launcher — screen set, navigation graph, knowledge-in-context, ambient Product context, identity-only launch handoff, no-poll refresh model, Bubble Tea v2 adapter, and Product-only query scope. |
