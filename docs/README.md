...
| [`terminal-launcher-contract.md`](./terminal-launcher-contract.md) | **Candidate C18:** read-only launcher — screen set, navigation graph, knowledge-in-context, ambient Product context, identity-only launch handoff, no-poll refresh model, and the open terminal rendering-dependency question. Non-authorizing. |
