...
func TestBlockedSessionsRoutesAttentionUnderBurst(t *testing.T) {
	...
	for _, sess := range sessions {
		...
		if _, err := s.DatabaseForTesting().Exec(`INSERT INTO agent_client_keys(client_ref,key_id,public_key,status,created_at) VALUES(?,?,?,?,?)`,
...
func insertChallenge(t *testing.T, s *Store, grantRef string, issued, expires time.Time, status, consequence string) {
	...
	_, err := s.DatabaseForTesting().Exec(`INSERT INTO agent_approval_challenges(challenge_ref,grant_ref,operation_digest,scope_json,version_json,consequence,host_assertion_digest,issued_at,expires_at,status,consumed_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
