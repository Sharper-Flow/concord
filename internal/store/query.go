...
func readQ4BlockerGraph(ctx context.Context, tx *sql.Tx, selected []WorkItem, depth, nodeLimit, edgeLimit int) ([]WorkItem, bool, bool, error) {
	...
	SELECT g.root_id,g.parent_id,g.blocker_id,g.depth,b.kind,b.title,b.lifecycle,b.priority,b.urgency,b.created_at,b.updated_at,coalesce(b.terminal_time,''),b.version
	FROM graph g
	...
	for rows.Next() {
		...
		var version int64
		if err := rows.Scan(&rootID, &parentID, &blockerID, &graphDepth, &kind, &title, &lifecycle, &priority, &urgency, &createdAt, &updatedAt, &terminalAt, &version); err != nil {
		...
		node := &q4GraphNode{item: WorkItem{ID: blockerID, Kind: kind, Title: title, Lifecycle: lifecycle, Version: version, Priority: int64(priority), Urgency: urgency, CreatedAt: createdAt, UpdatedAt: updatedAt, TerminalAt: terminalAt, Active: !terminalState(lifecycle), Terminal: terminalState(lifecycle)}, seen: map[string]bool{}}
