...
func seedKnowledgeWork(t *testing.T, s *Store, id, title string) {
	...
	events := []Event{
		...
		{EventID: "create-work-" + id, Kind: "work.created", SubjectType: SubjectWorkItem, SubjectID: id, Actor: "operator", OccurredAt: time.Now().UTC(), PayloadVersion: 1, Payload: []byte(`{"kind":"task","title":"` + title + `","priority":2}`)},
